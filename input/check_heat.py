...
def integration_levels(folders, filenames, level):
    ...
    int1 = []
    for i in range(n):
       heat_lon, heat = open_heat_static(folders[i], filenames[i])[2:]
...
def plot_vslat(folders, filenames, labels, colors):
    n, lat, int = integration_levels(folders, filenames, 'lat')
...
def plot_vsdefault(folders, filenames, labels, colors):
    n, lat, int = integration_levels(folders, filenames, 'full')
    ratios = []
    for i in range(n):
